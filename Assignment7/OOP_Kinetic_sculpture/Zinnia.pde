class Zinnia {  
  ...
  PShape zinniaBlade(PVector scl, int strokeWt, color zColor) {
    ...
    //creating shape with zinniaBlade and center
    PShape zinnia = createShape(GROUP);
    zinnia.addChild(zinniaBlade);
    if (zColor<220)
      fill(zColor+35);
    zinnia.addChild(createShape(ELLIPSE, 0, 0, 50, 50));
  ...
  void display(float thetaClockwise, float thetaCounterclockwise) {  
    ...
    //drawing opposite rotating zinnia half
    pushMatrix();
    translate(loc.x, loc.y);
    scale(-1, 1);
